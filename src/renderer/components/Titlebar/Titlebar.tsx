...
export default function Titlebar({ title, className }: TitlebarProps)
{
  return (
    <div className={`titlebar w-screen flex flex-row place-items-center text-xs bg-gray-100 bg-opacity-10 text-white ${className}`}>
      ...
      <div className='flex-none h-full no-drag flex flex-row w-28'>
        <button type='button' onClick={() => handleWindowAction('minimize')} className='flex-grow h-full hover:bg-white hover:bg-opacity-10 focus:ring-0'>
